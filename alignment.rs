...
fn priv_best_alignment<'a>(score_grid: &'a Vec<i32>,directions: &'a Vec<String>, cell: i32, seq1: &'a mut String, seq2: &'a mut String, aligned_seq1: &'a mut Vec<Vec<char>>, aligned_seq2: &'a mut Vec<Vec<char>>, aligned_seq_index: &'a mut usize) -> (Vec<String>, Vec<String>){
    ...
    let mut hasUp:bool = false;
    // Add to aligned sequences length
    ...
    if directions[direction_index].contains("L") {
        ...
        if hasDiagonal {
            ...
            *aligned_seq_index = *aligned_seq_index + 1;
            copy_aligned_seq1[*aligned_seq_index] = copy_aligned_seq1[*aligned_seq_index - 1 as usize].to_vec();
            copy_aligned_seq2[*aligned_seq_index] = copy_aligned_seq2[*aligned_seq_index - 1 as usize].to_vec();
        }
    ...
    if directions[direction_index].contains("U") {
        ...
        if hasDiagonal || hasLeft {
            ...
            *aligned_seq_index = *aligned_seq_index + 1;
            copy_aligned_seq1_2[*aligned_seq_index] = copy_aligned_seq1_2[*aligned_seq_index - 1 as usize].to_vec();
            copy_aligned_seq2_2[*aligned_seq_index] = copy_aligned_seq2_2[*aligned_seq_index - 1 as usize].to_vec();
        }
...
pub fn build_best_alignment<'a>(score_grid: &'a Vec<i32>,directions: &'a Vec<String>, cell: i32, seq1: &'a mut String, seq2: &'a mut String) -> (Vec<String>, Vec<String>){
    ...
    return priv_best_alignment(score_grid, directions, cell, seq1, seq2, &mut aligned_sequence1, &mut aligned_sequence2, &mut index);
}
